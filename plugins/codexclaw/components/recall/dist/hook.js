...
import { cwdMatches, FOLD_CWD_CASE } from "./rollout.js";
...
export function extractRecallTargets(prompt        , cap = TARGET_CAP)           {
  const out           = [];
  const seen = new Set        ();
  const push = (raw        )       => {
    const term = raw.trim();
...
function buildDirective(targets                    = [])         {
  const cxc = CXC();
  const rows = [
 ...
 */
export function dedicatedToolsEnabled(home         )          {
  try {
...
function memoriesTableBody(text        )                {
...
function recoveryLine(cxc        , dedicatedTools         )         {
 ...
 * ANSI.
 */
export function assertLegalHookResult(result            )       {
