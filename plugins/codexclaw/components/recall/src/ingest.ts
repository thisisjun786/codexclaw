...
function readSlice(path: string, from: number, to: number): Buffer {
...
export function ingest(home: string, db: RwDb, days = 0): IngestResult {
  ...
  const insertEntries = (path: string, entries: ReturnType<typeof parseRollout>, startOrd: number): number => {
    let ord = startOrd;
