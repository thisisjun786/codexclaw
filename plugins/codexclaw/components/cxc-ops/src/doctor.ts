 ...
 */
export const DOCTOR_DECLARED_FEATURES = ["multi_agent", "goals", "hooks", "default_mode_request_user_input"] as const;
export const DOCTOR_SOFT_FEATURES: ReadonlySet<string> = new Set(["default_mode_request_user_input"]);

/** Parse `codex features list`: `{name}  {stage}  {true|false}`, matching the first field exactly. */
export function parseDoctorFeatures(stdout: string): Map<string, boolean> {
  const declared = new Set<string>(DOCTOR_DECLARED_FEATURES);
...
export function buildDeclaredFeaturesCheck(res: { status: number | null; stdout: string; stderr: string }): CheckResult {
  if (res.status !== 0) {
    return {
      name: "features",
      severity: "WARN",
