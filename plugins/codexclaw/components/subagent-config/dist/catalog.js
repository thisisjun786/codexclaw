...
export function nativeCatalogPath(env                    = process.env)                {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const value         = match[1].startsWith("'") ? match[1].slice(1, -1) : JSON.parse(match[1]);
...
export function reasoningEfforts(raw         )                  {
  if (!Array.isArray(raw)) return null;
  return [...new Set(raw.flatMap(value => {
    const effort = typeof value === "string" ? value : value && typeof value === "object" ? (value                        ).effort : undefined;
